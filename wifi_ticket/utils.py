...
@frappe.whitelist()
def create_payouts():
    tickets = frappe.get_all("Wifi Ticket", {"payment_status": "Paid", "payment_method": "wave"}, ["*"])
    total = 0
    for ticket in tickets:
        if not ticket.payment_settled:
            continue
        amount = frappe.db.get_value("Wifi Plan", ticket.plan, "price")
        total += amount
    return total
